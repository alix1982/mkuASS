...
function Gallery (props) {
  ...
  const [currentPhotoPrevious, setCurrentPhotoPrevious] = useState(galleryArr[lengthGalleryArr - 1]);
  ...
  function switchingPhoto () {setTimeout(choosePhoto, 5000);}
  useEffect(switchingPhoto,[counterGallery]);

  function setPhotoGallery (endArray, valueEndArray) {
    galleryArr.forEach((item, index)=>{
      if (item === currentPhoto) {indexPhoto=index}
    });
    if (indexPhoto === endArray) { setCurrentPhoto(galleryArr[valueEndArray]) } 
    else { setCurrentPhoto(galleryArr[(endArray === (galleryArr.length-1)) ? (indexPhoto+1) : (indexPhoto -1)]) }
    clearTimeout(choosePhoto);
  }

  function setPhotoNext (endArray, valueEndArray) {
    galleryArr.forEach((item, index)=>{
      if (item === currentPhotoNext) {indexPhoto=index}
    });
    if (indexPhoto === endArray) { setCurrentPhotoNext(galleryArr[valueEndArray]) } 
    else { setCurrentPhotoNext(galleryArr[(endArray === (galleryArr.length-1)) ? (indexPhoto+1) : (indexPhoto -1)]) }
    clearTimeout(switchingPhoto)
  }
  
  function setPhotoPrevious (endArray, valueEndArray) {
    galleryArr.forEach((item, index)=>{
      if (item === currentPhotoPrevious) {indexPhoto=index}
    });
    if (indexPhoto === endArray) { setCurrentPhotoPrevious(galleryArr[valueEndArray]) } 
    else { setCurrentPhotoPrevious(galleryArr[(endArray === (galleryArr.length-1)) ? (indexPhoto+1) : (indexPhoto -1)]) }
    clearTimeout(switchingPhoto)
  }
  ...
  return (
      <section className="gallery">
        ...
        <button className="gallery__buttonLeft" onClick={onSwitchingPhotoLeft}>
          ...
          <img src={currentPhotoPrevious} className="gallery__buttonLeftImg" alt="предыдущее фото"/>
        </button>
        ...
        <button className="gallery__buttonRight" onClick={onSwitchingPhotoRight}>
          ...
          <img src={currentPhotoNext} className="gallery__buttonRightImg" alt="следующее фото"/>
        </button>
